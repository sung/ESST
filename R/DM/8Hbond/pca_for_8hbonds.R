...
##############################################################################
########## READ DATA #########################################################
dist_table <- read.table("./8hbonds.txt")
...
library(rgl);
cv<-cv.hb.n;
rgl::plot3d(pca$x, main='PCA for 8 H-bond types', type='n'); text3d(pca$x, text=rownames(pca$x),cex=1.0, col=palette(cv)); bg3d("lightgray");
